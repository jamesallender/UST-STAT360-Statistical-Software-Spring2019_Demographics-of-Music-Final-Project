...
diversity_g <- racially_diverse_music_2018_mod %>% filter(genre == "Classic Rock") %>% ggplot() + 
  geom_bar(mapping = aes(x = genre, y=avg_response), stat="identity", color="white", fill="blue", width=0.5) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))+
  labs(x = "", y = "") +
  theme(axis.text.x = element_blank())+
...
ggdraw() +
  draw_plot(preference_g + theme(legend.justification = "bottom"), 0, 0, .9, 1) +
  draw_plot(diversity_g, 0.8, 0, .15, 1) +
