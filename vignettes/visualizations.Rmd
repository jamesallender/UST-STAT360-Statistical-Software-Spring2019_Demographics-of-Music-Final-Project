...
It shows how the total preferences of all age ranges for a perticular genre of music.
```{r}
# Data 1 Graph 1
...
We thought that the original graph was a little messy and hard to understand, so we decided to use a faccet wrap to seperate the graphs based on age range to better ilistrate what the data is expressing.
```{r}
# Data 1 Graph 2
...
This is the same as graph 1, but with the x and y coordinates switched, which we thought was easier to visualize
```{r}
# Data 3 Graph 2
racially_diverse_music_2018 %>% ggplot() + 
  geom_bar(mapping = aes(x=genre, y=percent_response), stat="identity", color="white", fill="blue") +
...
This graph is the same as 2, but ordered so it's least to greatest.
```{r}
# Data 3 Graph 3
racially_diverse_music_2018 %>% ggplot() + 
...
These graphs represent each genre's likability compared to how diverse of a genre they are perceived to be.
```{r}
generate_plots <- function(str_genre){
  preference_g <- combined_data %>% filter(genre == str_genre) %>% ggplot() + 
    geom_bar(mapping = aes(x=age_range, y=percent_response, fill = age_range), stat="identity") +
    theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
